function [ixDG,RonEDG,nen,numnp,numelDG,nummat,MatTypeTable,MateT,NodeTable, ...
          PBCListNew,numPBCnew,NodesOnLinkNew,NodesOnLinknumNew,CouplerNodes] = ...
         FormMPCCZ(PBCListNew,numPBCnew,NodesOnLinkNew,NodesOnLinknumNew,CouplerNodes,...
          SurfacesI,ix,ixCG,RonE,NodeTable,numCL,nen_bulk,ndm,numnp,numel,nummat,maxmat, ...
          iel,nonlin,mateprop,PBCList,NodesOnLink,NodesOnLinknum,MatTypeTable,MateT)
%
% Tim Truster
% 03/25/2017
%
        ...
        for i = 1:length(CouplerSet1) % Form new links
            ...
            if NodesOnLinknumNew(nodeA) == 0
                ...
                if ~old
                    ...
                    node = CouplerNodes(iPBC);
                    CouplerSetM(nodeorder(i)) = abs(node);
                end
            end
        end
...
%         end
        for i = 1:numfnA
            ...
            for iPBC = 1:NodesOnLinknum(nodeA)
                jPBC = NodesOnLink(iPBC,nodeA)
                nodeCD = PBCList(jPBC,1:2) %#ok<NASGU>
            end
        end
